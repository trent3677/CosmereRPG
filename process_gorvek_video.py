#!/usr/bin/env python3
"""
Process Bandit Captain Gorvek Video
...
def find_video_file():
    """Find the Gorvek video file"""
    possible_names = [
        "bandit_captain_gorvek_animation.mp4",
        "bandit_captain_gorvek.mp4",
        "gorvek_animation.mp4",
        "gorvek.mp4",
        "captain_gorvek_animation.mp4",
        "captain_gorvek.mp4"
    ]
    ...
    mp4_files = [f for f in os.listdir('.') if f.endswith('.mp4') and 'gorvek' in f.lower()]
    if mp4_files:
        return mp4_files[0]
    
    # Also check for captain files
    mp4_files = [f for f in os.listdir('.') if f.endswith('.mp4') and 'captain' in f.lower()]
    if mp4_files:
        return mp4_files[0]
...
def main():
    ...
    print("="*60)
    print("BANDIT CAPTAIN GORVEK VIDEO PROCESSING")
    print("="*60)
    ...
    # Find the video file
    print("\nSearching for Bandit Captain Gorvek video...")
    input_file = find_video_file()
    
    if not input_file:
        print("[ERROR] Could not find Gorvek video file")
        print("Looking for files with 'gorvek' or 'captain' in the name")
        print("\nAvailable MP4 files:")
        for f in os.listdir('.'):
            if f.endswith('.mp4'):
                print(f"  - {f}")
    ...
    compressed_file = os.path.join(output_dir, "bandit_captain_gorvek_video.mp4")
    thumbnail_file = os.path.join(output_dir, "bandit_captain_gorvek_thumb.jpg")
    
    print(f"\n{'='*60}")
    print(f"Processing BANDIT CAPTAIN GORVEK (Half-Orc Boss)")
    print(f"{'='*60}")
    ...
    print(f"\n[OK] Successfully processed Bandit Captain Gorvek!")
    ...
    print("="*60)
    print("\nGorvek the Half-Orc Bandit Boss is ready for battle!")
    print("Video and thumbnail are ready in web/static/media/monsters/")
